...
fn bind_api_example() {
    {
        ...
        // `bind()` will try them one by one until one succeeds:
...
fn socket_api_example() {
    ...
    // Reading from a socket can be configured to return immediately.
    // Then, when there are no bytes, a `WouldBlock` error is returned:
    socket.set_nonblocking(true).unwrap();
}

fn run_client_server() {
    // We will use a channel to be notified when the server is ready
    // to accept requests:
    let (server_ready_notify, server_ready_recv) = unbounded();

    // Run the server in another thread:
    let thread = std::thread::spawn(move || server::echo_server(server_ready_notify));
