...
fn server_stream(stream: TcpStream) -> StreamOwned<ServerConnection, TcpStream> {
    ...
    let certs =
        rustls::pki_types::CertificateDer::pem_slice_iter(certs::SERVER_FULL_CHAIN.as_bytes())
            .flatten()
            .collect();
    ...
    let private_key =
        rustls::pki_types::PrivateKeyDer::from_pem_slice(certs::SERVER_PRIVATE_KEY.as_bytes())
            .unwrap();
    ...
    // Wrap the TCP system in TLS:
    StreamOwned::new(connection, stream)
}

// This client sends one message to the server and waits for the acknowledgment.
// For simplicity, the sizes of the messages are fixed:
fn client() {
...
// itself afterward. Moreover, the sizes of the messages are fixed:
fn server() {
